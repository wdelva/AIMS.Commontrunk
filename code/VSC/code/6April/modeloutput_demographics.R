...
modeloutput.all <- function(simpact.results = results, vsc = 0){
  ...
  # Recording the identifier of the simulation run
  id <- simpact.results$id
  # Reading the data
  ...
  # Calculating total time spent in relationships
  datalist$rtable$DisTimeTruncated <- pmin(datalist$rtable$DisTime, simpact.results$simulationtime)
  relDuration <- datalist$rtable$DisTimeTruncated - datalist$rtable$FormTime
  TotalRelDuration <- sum(relDuration)
  ...
  incidence.object <- incidenceheatmapdata.list(datalist)
  Exp.hivincids <- exp(incidence.object$incid.table$Incidence_All)
  # Calculating HIV incidence per gender and age group and time block
  hivinc.grouped <- incidence.age.time.grouped.list(datalist, group.width = 5)
  hivinc.g.a.wide <- hivinc.grouped$Incidence_G_A #ordered by time_i, then Gender, then Age.group
  # length is always 30 * (population.simtime - hiv.seedtime)/group.width
  # Calculating R over time (not R0)
  R.object <- allRtimestepsdata.list(datalist)
  Exp.Rs <- exp(R.object$R)
  ...
  #Recording degree distribution in the past X years, at timepoints a, b and c (default = in the past 1 year)
  timepoints.abc <- c(simpact.results$input.params$config$hivseed.time + 10,
                      (simpact.results$input.params$config$hivseed.time + 10 + simpact.results$input.params$config$population.simtime)/2,
                      simpact.results$input.params$config$population.simtime)
  # FIRST FOR HIV NEGATIVE PEOPLE
  hiv.status <- 0
  hist.densities.vect <- numeric()
  for (tps in timepoints.abc){
    tps.degree.df <- timewindow.degree.list(datalist = datalist,
                                            timepoint = tps,
                                            startpoint = tps-3,
                                            startage = 18,
                                            endage = 50,
                                            hiv.status = hiv.status)
    hist.breaks <- c(0:11, 10000)
    if (is.numeric(tps.degree.df$degree)){
      hist.data <- hist(tps.degree.df$degree, breaks = hist.breaks, right = FALSE)
      first.ten <- head(hist.data$density, -1)
      hist.densities <- c(first.ten, 1 - sum(first.ten))
    } else {
      hist.densities <- rep(NA, 12)
    } 
    hist.densities.vect <- c(hist.densities.vect, hist.densities)
  }
  hist.dens.vect.neg <- hist.densities.vect # This is the vector for HIV negative people
  # NOW FOR HIV POSITIVE PEOPLE
  hiv.status <- 1
  hist.densities.vect <- numeric()
  for (tps in timepoints.abc){
    tps.degree.df <- timewindow.degree.list(datalist = datalist,
  ...
  hist.dens.vect.pos <- hist.densities.vect # This is the vector for HIV negative people


  #Recording simulation time
  simulationtime <- simpact.results$simulationtime
  outputvector <- c(id, Exp.Growth.rate, TotalRelDuration, Log.PTR, Exp.hivprevs, Exp.hivincids, hivinc.g.a.wide, Exp.Rs, Exp.aAG, Exp.sdAG, Exp.slope, median.age.men, median.age.women, Exp.powerm, Exp.wsdad, Exp.bsdad, Exp.concurr.pointprev, hist.dens.vect.neg, hist.dens.vect.pos, simulationtime)
  return(outputvector)
}
