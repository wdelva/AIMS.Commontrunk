...
errFunction <- function(e)
{
  ...
  length.targetSS <- 3 * as.numeric(cfgABC["population.simtime"]) - as.numeric(cfgABC["hivseed.time"]) + 15 + 36 + 36 + 30 * (as.numeric(cfgABC["population.simtime"]) - as.numeric(cfgABC["hivseed.time"]))/5 #adding two times 3*12 densities of degreehistbins and 30 * (population.simtime - hiv.seedtime)/group.width HIV incidence estimates, grouped by age, gender and time block 

  #length.targetSS <- 335 # 101 HIV prevalence measurements + 14 other summary statistics + 110 HIV incidence + 110 R measurements
...
readthedata <- function(modeloutput){
  ...
  readetable <- read.csv.columns(eventlogfilename, has.header = FALSE, column.types = "rssiirsiir")
  etable <- setDT(readetable)
  etable.colnames <- colnames(etable)  
#etable <- fread(eventlogfilename, sep = ",", skip = 0)
  if (ncol(etable) == 10){
    setnames(etable, etable.colnames,
...
incidenceheatmapdata.list <- function(datalist){ #DTP = datalist$ptable, DTE = datalist$etable, cfg = configfile){
  ...
  for (time_i in head(timesteps, -1)){
    ...
    # (so we can be selective when plotting, and not plot if PY per age-gender group is too low)
    # setnames(Incidencetimestepdata, "V1", "Incidence") # Cases per personyear at risk
    if (nrow(Incidencetimestepdata) ==0){
      NArow.df <- data.frame(matrix(NA, ncol = length(names(Incidencetimestepdata)), nrow = 1))
      names(NArow.df) <- names(Incidencetimestepdata)
      Incidencetimestepdata <- rbind(Incidencetimestepdata, NArow.df)
    }
    Incidencetimestepdata <- cbind(time_i, Incidencetimestepdata)
    output <- rbind(output, Incidencetimestepdata)
  }
...
incidence.age.time.grouped.list <- function(datalist, group.width = 5){
  source("/user/data/gent/vsc400/vsc40070/simpact-test/code/datainput_demographics.R")
  DTP <- datalist$ptable
  DTE <- datalist$etable
  DTL <- datalist$ltable
  output <- data.table()
  timestep.width <- group.width
  decplaces <- decimalplaces(timestep.width)
  timesteps <- seq(from = as.numeric(cfgABC["hivseed.time"]),
                   to = as.numeric(cfgABC["population.simtime"]),
                   by = timestep.width)
  for (time_i in head(timesteps, -1)){
    DTcases <- incidentcase(DT = DTE, interval = c(time_i, (time_i + timestep.width)))  # New infections that happened at time x with: time_i < x <= time_(i+1)
    DTnonHIVdeaths <- nonHIVdeath(DT = DTE, interval = c(time_i, (time_i + timestep.width))) # New non-HIV deaths that happened at time x with: time_i < x <= time_(i+1)
    ...
    DTforincidencecalc$PYend <- pmin(DTforincidencecalc$eventtime.incident, DTforincidencecalc$eventtime.dead)
    DTforincidencecalc$PY <- DTforincidencecalc$PYend - time_i
    # Now we add binning of ages, in bins with width = group.width
    DTforincidencecalc$Age.group <- cut(DTforincidencecalc$Age, 
                                        breaks = c(seq(from = 0, to = 70, by = group.width), Inf), 
                                        labels = c("0-4", "5-9", "10-14",
                                                   "15-19", "20-24", "25-29", "30-34", "35-39",
                                                   "40-44", "45-49", "50-54", "55-59", "60-64",
                                                   "65-69", "70+"), 
                                        right = FALSE)
    Incidencetimestepdata <- DTforincidencecalc[ ,Incidence_G_A := sum(eventname.incident=="transmission", na.rm = TRUE) / sum(PY),by = "Gender,Age.group"] # And we calculate HIV Incidence, by gender and age group
    Incidencetimestepdata <- DTforincidencecalc[ ,PY_G_A := sum(PY),by = "Gender,Age.group"] # And we calculate PY, by gender and age group
    #Incidencetimestepdata <- DTforincidencecalc[ ,Incidence_G := sum(eventname.incident=="transmission", na.rm = TRUE) / sum(PY),by = "Gender"] # And we calculate HIV Incidence, by gender
    #Incidencetimestepdata <- DTforincidencecalc[ ,PY_G := sum(PY),by = "Gender"] # And we calculate PY, by gender
    #Incidencetimestepdata <- DTforincidencecalc[ ,Incidence_All := sum(eventname.incident=="transmission", na.rm = TRUE) / sum(PY)] # And we calculate HIV Incidence, by gender and age group
    #Incidencetimestepdata <- DTforincidencecalc[ ,PY_All := sum(PY)] # And we calculate PY, by gender and age group
    # (so we can be selective when plotting, and not plot if PY per age-gender group is too low)
    # setnames(Incidencetimestepdata, "V1", "Incidence") # Cases per personyear at risk
    if (nrow(Incidencetimestepdata) ==0){
      NArow <- rep(NA, length(names(Incidencetimestepdata)))
      NArow.df <- data.frame(NArow)
      names(NArow.df) <- names(Incidencetimestepdata)
      Incidencetimestepdata <- rbind(Incidencetimestepdata, NArow.df)
    }
    Incidencetimestepdata <- cbind(time_i, Incidencetimestepdata)
    incid.table <- unique(subset(Incidencetimestepdata, select = c(time_i, Age.group, Gender, Incidence_G_A, PY_G_A)))
    incid.table$cases <- incid.table$Incidence_G_A * incid.table$PY_G_A
    orderedincid.table <- setorder(incid.table, Gender, Age.group)
    output <- rbind(output, orderedincid.table)
  }
  return(output)
}
...
timewindow.degree.list <- function(datalist = datalist,
                                   timepoint = targetwindow.end,
                                   startpoint = targetwindow.start,
                                   startage = 18,
                                   endage = 50,
                                   hiv.status = 0){
  DTP <- datalist$ptable
  DTL <- datalist$ltable
  DTalive.infected <- alive.infected(DT = DTP, time = timepoint, dec.places = 0) # First we only take the data of people who were alive at timepoint
  DTalive.status <- subset(DTalive.infected, Infected == hiv.status)  
  DTage <- subset(DTalive.status,
                  Age >= startage & Age < endage)
  DTR <- subset(datalist$rtable,
                FormTime >= startpoint & FormTime < timepoint)
  timepoint.degreecalc <- timepoint
  DT.degree <- degreecalc(DT = DTage, DTR = DTR, timepoint = timepoint.degreecalc)# get lifetime degree of these people
  return(DT.degree)
}
...
agemixing.lme.fitter <- function(am.df){
  men.lme <- lme(AgeFemaleatForm ~ AgeMaleatForm0,
                 ...
                 random = ~1 | IDm, 
                 method = "REML",
                 weight = varPower(value = 0.7, form = ~AgeMaleatForm0 + 1))
}
