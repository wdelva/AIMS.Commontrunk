...
ncluster.1 <- 16#24#16#24 # nodes on muk have 16 cores, nodes on golett have 24 cores
...
cfg.conception.alpha_base <- -1.5 # Default is -3 which gives an intra-couple conception rate of 0.05. -2 gives a rate of 0.14
cfg.conception.alpha_agewoman <- -0.03 # effect of the age of woman in the relationship
...
preprior <- c(cfg.person.agegap.gender.dist.normal.sigma,
              ...
              cfg.conception.alpha_base,
              cfg.conception.alpha_agewoman,
              cfg.dissolution.alpha_4)

# These are updated priors, after selecting 15 good runs from the 2 X 3000 attempted runs (seeds 12000-15000)
simpact_prior <- list(c("unif", preprior[1] - 2, preprior[1] + 1.5),
                      c("unif", preprior[2] - 0.5, preprior[2] + 0.3),
                      c("unif", preprior[3] - 0.2, preprior[3] + 0.5),
                      c("unif", preprior[4] - 0.2, preprior[4] + 0.75), 
                      c("unif", preprior[5] - 9, preprior[5] + 10),
                      c("unif", preprior[6] + 0.5, preprior[6] + 1),
                      c("unif", preprior[7] - 0.7, preprior[7] + 0.2),
                      c("unif", preprior[8] - 0.05, preprior[8] + 0.025),
                      c("unif", preprior[9] - 0.5, preprior[9] + 1),
                      c("unif", preprior[10] - 0.02, preprior[10] + 0.025),
                      c("unif", preprior[11] - 0.2, preprior[11] + 0.05))
...
#####
ncols <- ncol(ABC_rejection_result$stats)
stats <- ABC_rejection_result$stats
...
###
param <- data.frame(ABC_rejection_result$param)
...
save(list=c("inputANDoutput.df", "simpact_prior", "targetSS"), file = paste0(DestDir, "inputANDoutput.1000.RData"))
